...
Server.prototype.serveDir = function (pathname, req, res, finish) {
    ...
    fs.stat(htmlIndex, function (e, stat) {
        if (!e) {
            ...
            if (pathname in this.indexStore) {
                streamFiles(this.indexStore[pathname].files);
            } else {
                // Stream a directory of files as a single file.
                fs.readFile(path.join(pathname, 'index.json'), function (e, contents) {
                    if (e) { return finish(404, {}) }
                    var index = JSON.parse(contents);
                    this.indexStore[pathname] = index;
...
Server.prototype.respond = function (pathname, status, _headers, files, stat, req, res, finish) {
    ...
    if ((clientMTime  || clientETag) &&
        ...
        if (this.cache && (key in this.store) &&
	          this.store[key].stat.mtime >= stat.mtime) {
            res.end(this.store[key].buffer);
